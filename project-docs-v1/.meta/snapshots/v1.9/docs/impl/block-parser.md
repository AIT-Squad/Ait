...
<!-- @id:impl-block-parser-overview -->
## 概述

`src/ait/block_parser.py` 实现 Markdown → Block 列表的解析。是所有上层模块（索引、版本管理、合并、上下文组装）的基础。

<!-- @ref:prd/block-system#prd-block-format rel:implements -->
<!-- @ref:prd/block-system#prd-block-parse-rule rel:implements -->

<!-- @id:impl-block-parser-api -->
## 公开 API
...
class ParsedFile:
    ...
    blocks: list[Block]
    refs: list["Ref"]     # 文件中所有 @ref 关联（含所属 block_id）

@dataclass(frozen=True)
class Ref:
    source_block_id: str  # @ref 所在的块 id
    target_file: str      # @ref:{file}#{id} 中的 file
    target_block_id: str  # @ref:{file}#{id} 中的 id
    rel: str              # rel:xxx
...
<!-- @id:impl-block-parser-algorithm -->
## 算法
...
<!-- @id:impl-block-parser-edge-cases -->
## 边界场景
...
| 文件没有任何 @id | blocks=[]，整个文件作为 file_header |
| @id 在行尾有内容 | 严格按正则匹配，行尾必须仅是注释 |
| Windows CRLF | 解析前统一 normalize 为 LF |

<!-- @id:impl-block-parser-tests -->
## 测试要点
