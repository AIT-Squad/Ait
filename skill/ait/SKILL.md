...
- 其余参考（`references/`）：`chunk-system.md`（@id/@ref/@extract）·`chunk-parser.md`（解析边界）·`index-system.md`（index/specgraph 语义）·`version-manager.md`（三态/confirm/reset）·`merge-engine.md`（按存在性逐 chunk merge，绝不丢 chunk）·`overview.md`（设计边界）。
