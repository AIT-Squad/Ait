...
- **--content 组装边界（复用优先，v2.87）**：`prd/fsd/tdd create` 的 `--content` 只应包含——① 根 chunk（`ROOT_CHUNK_REQUIRED` 强制必须出现，逐字带**自身正文**：概述/范围/目标与度量/反向要求；不带其余任何既有 chunk）；② 本次真正**新增或内容变化**的 chunk。未变化的既有 chunk（哪怕在讨论背景里看到）一律不要放——merge 按 chunk id 逐条合并，索引未出现的 chunk 在 baseline 天然保留，省略≠删除；`action=add` 塞入已存在 chunk 会在写入前被拒（`DUPLICATE_BASELINE_CHUNK`，零落盘）。**默认走 modify 复用既有 chunk**（能共用同一句用户故事就不新增，见「需求演进纪律」）；不要因为讨论背景里有一堆既有内容就整份复制重传——正例 v2.81（2 条索引记录，干净）／反例 v2.86（23 条记录、21 条空操作）。
...
> 关系四种：`derives`(PRD 根→FSD 根,派生,恰 1:1)、`decomposes`(FSD split→子 FSD 根,拆分)、`details`(叶 FSD split→TDD,细化)、`depends_on`(同父兄弟 split,依赖)。
...
| `/ait prd create\|confirm\|revert ...` | `project-docs/.ait/ait-cli prd ...`（新模型 PRD；create 需已有活动版本；link 已退役） | main |
...
| `INVALID_PROJECT_NAME` | `init --new-model --name` 含大写/空格/中文/`/`/`..` | --name 只接受小写字母数字下划线+短横分段（须能构成合法 chunk id）。 |
...
| `GIT_COMMIT_FAILED` | 真实 git 仓库里提交失败 | 已自动回滚（MERGE_ROLLBACK 包装）；修 git 状态后重试。非 git 环境不报错，merge 结果带 git:"unavailable"。 |
| `USAGE_ERROR` | 命令/参数拼写错误（未知命令、缺参、非法选项） | 输出仍是单个 JSON；按 error 提示改命令。 |
| `INVALID_FILE_NAME` | --file 越界（路径逃逸/.md 后缀/跨 kind） | 只接受本 kind 目录内的相对索引路径。 |
| `MODIFY_RENAME_COLLISION`/`DUPLICATE_OVERRIDES_TARGET` | modify 改名撞已存在 id / 两记录撞同一 override 目标 | 前置拦截零落盘；修 overrides 后重试。 |
...
- 新模型关系不从命名推断，四种关系各随其创建命令原子出生（四关系四出生地）：derives 随 `fsd create --parent`、decomposes 随 `fsd decompose`、details 随 `tdd create --parent`、depends_on 随 `fsd create` 的 split 内 yaml 声明；无任何 link/depend 命令；每个 TDD 唯一映射一个 `target_file`。fsd/tdd create 对不存在的版本报 VERSION_NOT_FOUND。
