...
/ait fsd create <[FSD]-id> ...         → FSD 功能分解（根/内部 split，纯写文档）
...
| `/ait prd create\|confirm\|revert\|link ...` | `project-docs/.ait/ait-cli prd ...`（新模型 PRD；create 自动开版本） | main |
...
- 新模型关系不从命名推断，由 `fsd decompose`（decomposes）/ tdd create（details）在写内容时原子建边，无独立 link 命令；每个 TDD 唯一映射一个 `target_file`。
