...
**需求 chunk 的新增与 modify 判据（默认 modify）**：
- **判据**：新诉求能否与某个既有需求 chunk **共用同一句用户故事**（角色相同 + 业务价值相同）→ 能则以新增一条验收标准的方式 `modify` 该 chunk；不能才新增 chunk，并须在讨论中记录不能 modify 的理由。默认 modify——走新增时人会对旧文视而不见，历史上因此让两条互斥规格在 baseline 并存跨越四个版本。
- **拆分上限（可观察信号，非主观感觉）**：验收标准超过约 10 条，或该 chunk 的 `derives` 覆盖超过 3 个功能域，才考虑拆分；拆分须**显式迁移**（建新 chunk 并从原 chunk 移出对应条目），不得以"新增近亲 chunk"代替。
- **取代关系双向可读**：被取代的验收标准标注管辖权移交，取代方标注取代事实；不容许互斥规格并存。
- **derives 与 M:N**：需求 chunk 承载范围扩展时补建 `derives` 边即可（§4 的两级基数：PRD 需求→FSD split 为 M:N）。"关系需要维护"不构成新开需求 chunk 的理由。
- **id 稳定性**：internal split **不支持改名**（只有根 chunk 可经 `--overrides` 改名）。需求 chunk 的 id 是稳定锚点，承载范围的演进由标题与正文表达。
...
- **add = 仅新增** baseline 不存在的 chunk；命中已存在报 `DUPLICATE_BASELINE_CHUNK`（v2.87起`prd/fsd/tdd create` 写入时即零写入拦截，不必等到 merge 才发现——`--content` 只应携带真正新增的 chunk，勿把未改动的既有 chunk 一并粘贴进来）。
...
| 写时门禁 | `MISSING_ENDPOINT` `TDD_MULTI_PARENT` `PRD_FSD_LINK_NOT_UNIQUE` `DUPLICATE_TARGET_FILE` `TDD_TARGET_FILE_REQUIRED` `VERSION_NOT_FOUND` `CONTEXT_TOKEN_REQUIRED` `CONTEXT_TOKEN_INVALID` `CONTEXT_TOKEN_STALE` `CONTEXT_TOKEN_CONFLICT` `CONTEXT_SKIP_NOT_ALLOWED` `PRD_REQUIREMENT_CONTRACT_VIOLATION` `DUPLICATE_BASELINE_CHUNK` |
