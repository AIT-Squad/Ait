...
<!-- @summary: AIT——面向 AI 协作的 chunk 级规格版本控制与实现编排系统 -->
...
<!-- @summary: 以 chunk 为单位的三态版本控制,版本是全有或全无的原子单元 -->
...
<!-- @summary: 与 AI 讨论逐层分解,从需求走到代码 -->

**用户故事:** 作为 AI 协作开发者,我希望从 PRD 出发与 AI 讨论逐层分解为 FSD 功能树、再细化为文件级 TDD、最后由 codegen 驱动编码,以便让需求到代码的每一步都结构化、可门禁。

#### 验收标准
1. WHEN 依次 prd create → fsd create/decompose → tdd create --parent → codegen prepare THEN 每层文档写入版本工作区,关系边随内容创建原子出生,codegen 输出目标文件的聚焦上下文。
...
<!-- @summary: 每个制品沿 TDD→FSD→PRD 唯一可追溯,codegen 一跳组装上下文 -->
...
<!-- @summary: FSD 分区把系统拆成互不重叠子树,每个制品唯一属主 -->
...
<!-- @summary: 六条规格治理不变式由 AIT 自身门禁强制,非文档约定 -->
