...
<!-- @summary: AIT 产品需求：AI 时代 chunk 级文档版本控制 + 双流水线 + spec→代码可追溯 + FSD 可拆分并行。 -->
...
旧模型：PRD（需求）→ impl（实现设计，含 @extract 动态 global）→ task（AI 编码单元）→ code。新模型：PRD → 递归 FSD（功能分解树）→ 叶 FSD `details` TDD（一文件一映射）→ codegen → code。两模型并存，旧命令不动；新模型用 prdv2/fsd/tdd/codegen。验收：两条流水线分别可端到端跑通。
