...
### 代码结构（FSD 依赖声明，v2.26）
- **依赖随内容出生**：FSD 冒号 split 的兄弟依赖在 split chunk 正文的 yaml 块里声明——`depends_on: [store, config]`（仿 TDD `target_file` 先例；简写按同父解析 `store → {root_id}:store`，也接受完整 chunk_id 但必须同父）。specgraph 仍是唯一权威存储，内容是申报来源；无独立 depend/link 命令。
- `_split_depends_on(chunk_content) -> list[str]`：在 chunk 正文中找 ```yaml 围栏块，safe_load 取 `depends_on` 列表；无块/无键 → []。
- `_reconcile_sibling_depends_on(version, root_id, split_ids, declared)`：**owned-scope 全量对账**——同父约束使全部合法 depends_on 边都是"本文件兄弟边"，文件＝依赖边的所有权边界。在版本图中：删除 src∈本文件 split 的全部 depends_on 边，再按声明重建（metadata source="fsd-declaration"，经 `_preserve_explicit_edges` 跨 reindex 存活）。**必须在 `_create_document` 的 sync_specgraph 之后执行**（sync 保留旧显式边，对账负责覆盖）。幂等：同声明重跑结果不变；改依赖＝modify 文件改声明，被删声明的边随对账消失。
...
- `create_fsd/create_prd(version,root_chunk_id,content,*,file=None,action="add",overrides=None)`：均委托 `_create_document(kind)`；**create_prd 是流转入口：写入后若 meta.phase ∈ {None,empty} 置为 `prd-creating`（阶段机起点）**；**create_fsd 写入后若 phase 为 prd-confirm 则推进为 `fsd-creating`（FSD 层起点）；create_fsd 解析全部 split 的 depends_on 声明（先校验后写入），写盘+sync 后执行 `_reconcile_sibling_depends_on`**；`create_tdd` 先校验含 target_file（否则 `TDD_TARGET_FILE_REQUIRED`），**再做写时唯一性门禁：normalize_target_file 后与组合视图内既有 TDD 属主比对（经 `collect_tdd_target_files`），撞车且属主不是本 chunk（modify 自身豁免）→ `DUPLICATE_TARGET_FILE` 拒绝，不写入、可改路径重试**。
- `add_edge(version,src,dst,rel)`：**底层建边原语（不再直接暴露为 CLI；decomposes 归 `decompose_fsd`，link 命令退役）**。rel 不在 NEW_MODEL_RELS → `INVALID_NEW_MODEL_REL`；**写时局部门禁：view=combined_view(root,version)，`check_edge_write(view,src,dst,rel)` 有违例 → 以首条违例 code 抛 ValidationError（幽灵端点 `MISSING_ENDPOINT`／TDD 第二父 `TDD_MULTI_PARENT`／PRD 第二 FSD `PRD_FSD_LINK_NOT_UNIQUE`），不落盘、修正后可重试**；通过后 combined 图解析 src/dst URI；load 版本图 add_edge(metadata source="new-model-cli") save。全局完整性（孤儿/断链/环）不在写时查，归 version confirm 门禁。
- `decompose_fsd(version, parent_chunk_id, child_root_chunk_id, *, content=None, file=None) -> EdgeCreateResult`：**FSD 层"拆分即建边"统一入口，取代 fsd link 的 decomposes 用法**。①`_precheck_decompose_parent(view,parent,child)`——parent 不在组合视图 → `MISSING_ENDPOINT`；parent 为 PRD 且已有 decomposes 出边指向非 child 的 FSD → `PRD_FSD_LINK_NOT_UNIQUE`（父侧门禁前置于任何写入，保拒绝＝零落盘）；②content 非空 → `create_fsd(version, child_root, content, file=file)` 原子写子 FSD（走 create 全校验）；③`add_edge(version, parent, child_root, "decomposes")` 建边（此时两端点在视图，check_edge_write 完整过）；④phase 若为 prd-confirm 推进为 fsd-creating。rel 恒 decomposes（details 归 tdd 层）。返回 EdgeCreateResult。
...
- `_create_document(version,root_chunk_id,content,*,kind,file,action,overrides)`：file 缺省 `{kind}/{root_chunk_id}`；parse_text 校验含 root chunk（否则 `ROOT_CHUNK_REQUIRED`）；**版本入口收口（R3-04 完全体）：kind∈{fsd,tdd} 时版本 meta 必须已存在，否则 `VERSION_NOT_FOUND`（提示先 `version create` 或 `prd create`）——只有 prd 是版本入口层，保留 `ensure` 语义**；write_version_file；re-parse；逐 chunk add_chunk（overrides 仅给 root）；sync_specgraph。
...
- `next_version_name()`：扫描全部版本名 `v{major}.{minor}` 取最大后 minor+1；无任何版本 → `v0.1`。供 CLI `prd create` 无活动版本时自动开版本。

### 模块函数
`_target_file(text)`(正则)、`_parent_chunk_id`(split ":" 前)、`_file_stem`(rsplit "/")、`_validation_error(code,message,chunk_id)`。

### 错误码
`TDD_TARGET_FILE_REQUIRED`、`INVALID_NEW_MODEL_REL`、`TDD_NOT_FOUND`、`ROOT_CHUNK_REQUIRED`、`NO_PRD_CHUNKS`、`NO_FSD_CHUNKS`、`NO_TDD_CHUNKS`、**`VERSION_NOT_FOUND`、`DEPENDS_ON_UNKNOWN_SIBLING`、`DEPENDS_ON_SELF`**；写时门禁转发校验器码：`MISSING_ENDPOINT`、`TDD_MULTI_PARENT`、`PRD_FSD_LINK_NOT_UNIQUE`、`DUPLICATE_TARGET_FILE`、`DEPENDS_ON_CROSS_LEVEL`。
...
### 单元测试要求
`tests/test_new_model_commands.py`、`test_new_model_merge.py`、`test_v21_toolchain.py`、`test_v22_new_model_lifecycle.py`：create_prd/fsd/tdd、add_edge 三关系、prepare_codegen 上溯+baseline 回退、collect_tdd_target_files；prepare_codegen 对 decomposes 环图与自环的上溯短路（不爆栈、每节点至多一次）；回归：TDD 经 --action modify 进版本后 prepare_codegen(version) 的 upstream 为完整链、dependencies 非空（v2.18 缺口）；**写时门禁：add_edge 幽灵端点/TDD 第二 details 父/PRD 第二 decomposes 各拒绝且零落盘、修正后重试成功；create_tdd 归一化撞车（`./`、反斜杠、大小写变体）拒绝、换路径重试成功、modify 自身同 target 豁免**；**PRD 层流转：create_prd 置 phase 起点、confirm_prd_layer 锁 PRD chunk+phase、revert_prd_layer 解锁回 working+phase 回退、merged 拒 revert、next_version_name 递增与空库缺省**；**FSD 层：decompose_fsd 原子写子 FSD+建 decomposes 边、父侧预检拒绝时零落盘、confirm/revert 冻结-返工、NO_FSD_CHUNKS**；**TDD 层：create_tdd --parent 原子建 details 边、TDD 第二父拒绝零落盘、幽灵 parent 拒绝、confirm_tdd_layer 锁 TDD chunk+phase→tdd-confirm、revert_tdd_layer 解锁+phase→tdd-creating、NO_TDD_CHUNKS；端到端 prd→fsd decompose→tdd create --parent→codegen prepare 上溯链非空**；**依赖声明（v2.26）：声明→版本图生边（简写解析+完整 id）、未知兄弟/自依赖/跨父各拒且零落盘、modify 改声明→对账后新增边在删除边亡、同声明幂等、fsd/tdd create 对不存在版本报 VERSION_NOT_FOUND（prd 保留 ensure）**。pytest。
