...
- `VersionMeta`：version、created_at、merged_at?、owner="system"、changes[]、dependencies、snapshot?、**phase=empty、title?、prd_locked=False、impl_locked=False**（原子性/锁定字段，default 使旧 meta 仍可加载）。
...
### 单元测试要求
`tests/`（schema 相关）：round-trip、extra 键被拒、summary>120 报错、delete entry 允许 null 字段、旧 version meta（无 phase）仍加载。pytest。
