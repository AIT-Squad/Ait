...
- `NEW_MODEL_CHUNK_ID = r"\[(?:PRD|FSD|TDD)\]-[a-z0-9_]+(?:-[a-z0-9_]+)*(?::(?:[a-z0-9_]+|TEST))?"`（**v2.32：冒号 split 段允许小写功能名 或 保留大写标记 `TEST`——`:TEST` 是每个 FSD 文件的验收节点 chunk,大写醒目区别于功能 split;仅此一个保留大写 token**）
...
### 单元测试要求
`tests/test_chunk_parser.py`：@id 边界、code-fence 屏蔽、bracket 前缀 ID、内部 split `:`、@ref 归属、summary/no-impl 剥离、@extract 嵌套/未闭合报错;**`:TEST` 保留大写标记可解析、普通 split 仍限小写(`:Foo` 拒)**。pytest。
