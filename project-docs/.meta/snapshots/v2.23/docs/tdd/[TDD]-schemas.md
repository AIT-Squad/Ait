...
### 单元测试要求
`tests/`（schema 相关）：round-trip、extra 键被拒、summary>120 报错、delete entry 允许 null 字段、旧 version meta（无 phase）仍加载；新模型 phase 值（prd-creating/prd-confirm/fsd-creating/fsd-confirm）round-trip。pytest。
