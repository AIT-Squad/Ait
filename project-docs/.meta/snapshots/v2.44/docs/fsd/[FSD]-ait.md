...
- **接口**:
  - `run(*, check_only=False, skip=(), new_model=False, project_name="project") -> InitResult(created_files, chunks, specs, skill_dir, cli_path, wrapper_path, status, files, skipped)`——fresh/incomplete/ready 状态判定;new_model=True 生成 docs/{prd,fsd,tdd} 与 [PRD]/[FSD] 根 + decomposes 边(直接入 specgraph,PRD 正文零关系);check_only 只报告
...
### 功能描述
新模型主线(PRD→FSD→TDD→codegen)的引擎:创建三类文档、关系随内容创建原子出生、六不变式校验内核、codegen 上下文组装。关系出生的三条路:depends_on 随 fsd create 的 split 内临时 yaml 声明(建边后从正文剥离,文档零关系声明)、decomposes 随 fsd decompose(拆分即建边)、details 随 tdd create --parent(创建即建边);无任何 link/depend 命令,幽灵边从入口消灭。内部分解见 [FSD]-ait-new_model。
...
- **接口**:
  ...
  - `fsd create <id> --content*`——FSD 写入;解析 split 内 depends_on 声明→建边→剥离;`fsd decompose <parent> <child> [--content]`——拆分即建 decomposes 边(父侧预检 MISSING_ENDPOINT/PRD_FSD_LINK_NOT_UNIQUE);`fsd confirm` / `fsd revert`
