...
- `prepare_codegen(version,tdd_root)`：version=None 直接 baseline 解析；否则 query_version 缺则回退 baseline；读 target_file（root.content 或全文）缺→`TDD_TARGET_FILE_REQUIRED`；combined 图解析 tdd_uri→`_collect_upstream_context`→`_collect_dependency_context`；返回 CodegenBundle。
...
- `_collect_dependency_context(graph,parent_split_uri)`：从父 split 出发的 depends_on 边→兄弟 split→其 decomposes/details 子。
