...
### 单元测试要求
tests/test_install_py.py 及 init/new-model 相关：fresh/incremental/new-model bootstrap、幂等、decomposes 边、refresh-wrapper；**非法 name(大写/空格/中文/含 `/`/`..`)→INVALID_PROJECT_NAME、合法 name→chunks≥2 且根 chunk 落地、假 chunks==0 场景→BOOTSTRAP_FAILED；**v2.31：new-model bootstrap 后 PRD 正文不含 @ref、baseline 有 PRD→FSD decomposes 边(source=new-model-cli)**。pytest。
