...
- **组合视图**：`combined_view(root,version)` → `CombinedView`——chunk 身份=chunk_id，版本覆盖 baseline（modify=同节点换内容来源），边端点坍缩为 chunk_id 并去重；`edges_from/edges_to/impacted/detect_cycle`。被 modify 进版本的 chunk 在视图中保有 baseline 期建立的全部关系（消除 URI 二象性）；depends_on 边按 owned-scope 覆盖——版本内出现的 FSD 根，其兄弟依赖边以版本为准，声明删除立即生效。
