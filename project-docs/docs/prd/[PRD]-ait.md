...
<!-- @id:[PRD]-ait:iteration_continuity -->
### 需求:迭代连续性——现状驱动的分层讨论

**用户故事:** 作为 AI 协作开发者,我希望每层文档的创作/修改讨论都自动获得"沿关联检索出的系统现状"作为背景,以便结合现状与我的修改方向形成新内容,保证修改的连续性和可持续性。

#### 验收标准
1. WHEN 在任一层发起创作而未提供内容 THEN 系统 SHALL 返回该层的讨论背景——本版本上层改动的全文、沿关联检索出的现状全文、目标既有内容(如存在)——且零写入。
2. WHEN 针对一个已命名的分解/细化锚点(父块)发起而未提供内容 THEN 背景 SHALL 以该父块为中心:父块全文、其全部相邻块、其向上到需求的链条、既有子块。
3. WHEN 项目初始(基线为空) THEN 同一逻辑 SHALL 返回空背景——初始是"现状为空的迭代",逻辑零分支;初始化 SHALL 从一开始就建立空的基线存储。
4. 背景 SHALL 由系统沿关联自动提取组装,无需人工翻找文件;关联的结构完整性(六不变式)是检索可靠性的前提。

<!-- @id:[PRD]-ait:blank_slate_bootstrap -->
### 需求:白板起步——从空目录初始化项目

**用户故事:** 作为首次采用 AIT 的开发者,我希望在一个还没有 project-docs 的空目录里直接运行 init 就能把项目初始化好,以便无需手工预建任何目录即可开始第一个版本的迭代。

#### 验收标准
1. WHEN 在不含 project-docs 的目录运行 init THEN 系统 SHALL 自建 project-docs 骨架(docs 与元数据存储目录)、空基线存储,并完成基线引导,不因缺少目录而拒绝。
2. WHEN init 之外的命令在不含 project-docs 的目录运行 THEN 系统 SHALL 仍要求先有项目根(仅 init 是白板入口)。
3. WHEN init 后从零走 version→prd→fsd→tdd→codegen→merge THEN 全流程 SHALL 无需任何手工建目录即可推进到合入。
4. WHEN 对已初始化项目重跑 init THEN SHALL 幂等,不破坏既有内容。

<!-- @id:[PRD]-ait:docs_code_isolation -->
### 需求:docs/code 隔离仓——文档与制品独立的 git 历史

**用户故事:** 作为开发者,我希望 AIT 的文档历史与宿主代码历史完全独立,以便可以丢弃不满意的代码制品、保留规格文档、重新 codegen——而不丢失任何文档版本记录。

#### 验收标准
1. WHEN 运行 init THEN 系统 SHALL 在 project-docs 内建立独立 git 仓(.git),并在宿主根 .gitignore 追加 project-docs/,使文档历史与宿主历史完全隔离。
2. WHEN 版本进行期间(prd/fsd/tdd create)写入文档工作区文件 THEN 系统 SHALL 不拦截该操作——docs 仓在版本生命周期内故意处于 dirty 状态是合法的,confirm 不因此失败。
3. WHEN version merge 成功 THEN 系统 SHALL 原子提交 docs 仓所有变更并在 version meta 记录 docs_commit(docs 仓 commit sha)与 code_base(只读宿主 HEAD),两者作为跨仓版本绑定的语义锚点。
4. WHEN 代码制品不满意时执行不对称回滚 THEN 宿主仓 reset 到 code_base 扔掉制品,文档仓 SHALL 保持不动,下次 codegen 直接从现有规格重新生成。
5. IF 宿主非 git 仓(初始或纯文档项目)THEN code_base = null,merge 仍正常完成;docs 仓有独立 git 即满足核心需求。

<!-- @id:[PRD]-ait:specgraph_visualization -->
### 需求:规格图可视化——生成 Mermaid chunk 关系图

**用户故事:** 作为开发者,我希望运行一条命令就能生成一个 Markdown 文件,直观展示所有 chunk 节点及其关联关系,以便在 GitHub/IDE 里渲染查看完整的规格树结构。

#### 验收标准
1. WHEN 运行 `ait specgraph graph-md` THEN 系统 SHALL 在 `project-docs/docs/graph.md` 写入 Mermaid 图:每个 chunk id 是一个节点,同一 .md 文件的 chunk 聚合在同名 subgraph 框内,边标注 derives/decomposes/details/depends_on 关系类型。使用 baseline 数据。
2. WHEN 运行 `ait specgraph graph-md --version <v>` THEN 系统 SHALL 在 `project-docs/versions/<v>/graph.md` 写入同格式图,使用该版本的 combined_view(baseline ∪ 版本)数据。
3. WHEN chunk id 含 Mermaid 非法字符(`[`,`]`,`:`,`-`)THEN 系统 SHALL 自动转义为合法 node id,原始 chunk id 作为 node label 显示。
4. WHEN 文件已存在 THEN 系统 SHALL 直接覆盖写入(幂等)。
5. WHEN 项目无 specgraph 数据 THEN 系统 SHALL 输出空图(仅 Mermaid 骨架),不报错。
