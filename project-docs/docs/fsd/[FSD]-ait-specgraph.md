...
- impact 叶(details → [TDD]-impact)
- graph_md 叶(details → [TDD]-graph_md):Mermaid 规格图生成与写文件:下游影响分析命令
...
6. WHEN `graph_md(root)` THEN 返回含 `graph TD`、`subgraph`、edge 行的 Markdown 字符串,并将其写入 `docs/graph.md`;同 chunk id 在 subgraph 内出现一次;Mermaid node id 无非法字符。

<!-- @id:[FSD]-ait-specgraph:graph_md -->
## graph_md
### 功能描述
Mermaid 规格图生成。`generate_graph_md(root, version=None) -> str`:从 specgraph(baseline 或 combined_view)取全部节点与边,按 spec.file 分组,生成 Mermaid `graph TD` + `subgraph` 格式字符串——同文件 chunk 聚合进 `subgraph "file"` 框,边标注 derives/decomposes/details/depends_on。chunk id 中的 `[`,`]`,`:`,`-` 转义为合法 Mermaid node id,原 id 作 label。`write_graph_md(root, version=None) -> dict`:调前者后原子写入固定路径(baseline → `docs/graph.md`;version → `versions/<v>/graph.md`),返回 path/nodes/edges 统计。无数据时输出空 Mermaid 骨架不报错。

### 能力契约
- **提供方式**:Python 模块函数(ait.graph_md)
- **接口**:
  - `generate_graph_md(root:Path, version:str|None=None) -> str`
  - `write_graph_md(root:Path, version:str|None=None) -> dict{path,nodes,edges}`

### 反向要求
- 不加载或修改 specgraph 边(只读查询)。
- 不做 CLI 参数解析(归 cli.py)。
