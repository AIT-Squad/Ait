"""Tests for ait.block_parser."""

from __future__ import annotations

from pathlib import Path

from ait.block_parser import parse_file, parse_text

FIXTURE_ROOT = Path(__file__).parent.parent / "project-demo"


def test_parse_book_management_prd():
    """The demo PRD should yield 14 blocks (per blocks-index.yaml)."""
    ...
    assert len(pf.blocks) == 14
    block_ids = [b.id for b in pf.blocks]
    assert "prd-book-mgmt-overview" in block_ids
    assert "prd-book-lifecycle" in block_ids
...
"""
    ...
    assert [b.id for b in pf.blocks] == ["parent-x", "parent-x-child"]
    parent = pf.blocks[0]
    child = pf.blocks[1]
...
def test_crlf_normalized():
    ...
    assert len(pf.blocks) == 1
    assert "\r" not in pf.blocks[0].content
...
"""
    ...
    assert all(r.source_block_id == "impl-x" for r in pf.refs)
